(() => {
  ...
  function campaignCard(campaign) {
    ...
    if (campaign.description) {
      ...
      const desc = document.createElement("p");
      desc.textContent = "Sem descrição ainda.";
      article.appendChild(desc);
    }
    ...
    action.addEventListener("click", () => {
      setStatus(userStatus, "A entrada na mesa será conectada na próxima etapa.");
    });
  ...
  async function loadTables() {
    ...
    if (!data || data.length === 0) {
      ...
      box.className = "empty-state card";
      const title = document.createElement("h3");
      title.textContent = "Sua estante de mesas ainda está vazia";
      const text = document.createElement("p");
      text.textContent = "Quando você criar uma campanha ou aceitar um convite, ela vai aparecer aqui.";
      const button = document.createElement("button");
      button.type = "button";
      button.className = "button";
      button.textContent = "Criar minha primeira mesa";
      button.addEventListener("click", () => {
        setStatus(userStatus, "A criação de mesas será a próxima tela que vamos construir.");
      });
  ...
  accountToggle.addEventListener("click", () => {
    ...
    if (opening) accountPanel.querySelector("h2").focus?.();
  });

  createTable.addEventListener("click", () => {
    setStatus(userStatus, "A criação de mesas é a próxima etapa.");
  });
